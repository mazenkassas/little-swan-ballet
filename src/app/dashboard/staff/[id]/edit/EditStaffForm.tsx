...
const LABELS = {
  en: {
    back: 'Back', title: 'Edit Staff Member', sub: 'Update staff member details',
    nameAr: 'Full Name (Arabic)', nameEn: 'Full Name (English)',
    role: 'Role', roleDefault: 'Select role',
    email: 'Email Address', password: 'New Password',
    passwordHint: 'Leave blank to keep current password',
    phone: 'Phone Number', rate: 'Monthly Salary (EGP)', status: 'Status',
    roles: { super_admin: 'Super Admin', admin: 'Admin', staff: 'Staff' },
    save: 'Save Changes', saving: 'Saving…', cancel: 'Cancel',
    active: 'Active', inactive: 'Inactive',
    err: {
      name_ar: 'Arabic name is required', name_ar_alpha: 'Arabic letters only',
      name_en: 'English name is required', name_en_alpha: 'English letters only',
      role: 'Role is required',
      email: 'Email is required', email_format: 'Enter a valid email',
      password_min: 'Minimum 6 characters',
      phone: 'Phone is required', phone_digits: 'Digits only (0–9)',
      phone_starts_01: 'Must start with 01', phone_11: 'Must be 11 digits',
      hourly_rate: 'Salary is required', hourly_rate_pos: 'Must be greater than 0',
    },
  },
  ar: {
    back: 'رجوع', title: 'تعديل موظف', sub: 'تحديث بيانات الموظف',
    nameAr: 'الاسم بالعربي', nameEn: 'الاسم بالإنجليزي',
    role: 'الدور الوظيفي', roleDefault: 'اختر الدور',
    email: 'البريد الإلكتروني', password: 'كلمة المرور الجديدة',
    passwordHint: 'اتركها فارغة للحفاظ على كلمة المرور الحالية',
    phone: 'رقم الهاتف', rate: 'الراتب الشهري (جنيه)', status: 'الحالة',
    ...
    err: {
      name_ar: 'الاسم بالعربي مطلوب', name_ar_alpha: 'حروف عربية فقط',
      name_en: 'الاسم بالإنجليزي مطلوب', name_en_alpha: 'حروف إنجليزية فقط',
      role: 'الدور مطلوب',
      email: 'البريد مطلوب', email_format: 'أدخل بريدًا صحيحًا',
      password_min: 'على الأقل 6 أحرف',
      phone: 'الهاتف مطلوب', phone_digits: 'أرقام إنجليزية فقط',
      phone_starts_01: 'يبدأ بـ 01', phone_11: '11 رقمًا بالضبط',
      hourly_rate: 'الراتب مطلوب', hourly_rate_pos: 'يجب أن يكون أكبر من صفر',
    },
  },
}
...
export default function EditStaffForm({ staff }: { staff: StaffRecord }) {
  ...
  const [form,         setForm]         = useState({
    ...
    password:    '',
    phone:       staff.phone           || '',
    hourly_rate: staff.monthly_salary != null ? String(staff.monthly_salary) : '',
  })
  ...
  function validateField(field: FormFields, value: string): string | undefined {
    switch (field) {
      ...
      case 'email':
        ...
        return undefined
      case 'password':
        if (value && value.length < 6) return L.err.password_min
        return undefined
      case 'phone':
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (!isSuperAdmin && form.hourly_rate) payload.monthly_salary = parseFloat(form.hourly_rate)
    if (form.password) payload.password_hash = form.password

    const { error: err } = await supabase.from('staff').update(payload).eq('id', staff.id)
      ...
      <div style={{ padding: '24px 28px' }}>
        <form onSubmit={handleSubmit} noValidate style={{ maxWidth: 640 }}>
          <div style={{
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14, marginBottom: 14 }}>
              <div>
                ...
                <input value={form.name_ar} onChange={e => set('name_ar', e.target.value)} onBlur={() => touch('name_ar')} dir="rtl" style={inpStyle('name_ar')} />
                <Err field="name_ar" />
              </div>
              <div>
                <label style={lbl}>{L.nameEn}{req}</label>
                <input value={form.name_en} onChange={e => set('name_en', e.target.value)} onBlur={() => touch('name_en')} dir="ltr" style={inpStyle('name_en')} />
                <Err field="name_en" />
              </div>
            </div>
            ...
            <div style={{ marginBottom: 14 }}>
              ...
              <select value={form.role} onChange={e => { set('role', e.target.value); if (e.target.value === 'super_admin') setErrors(err => ({ ...err, hourly_rate: undefined })) }} onBlur={() => touch('role')} style={{ ...inpStyle('role'), cursor: 'pointer' }}>
            ...
            <div style={{ marginBottom: 14 }}>
              ...
              <input type="email" value={form.email} onChange={e => set('email', e.target.value)} onBlur={() => touch('email')} dir="ltr" autoComplete="off" placeholder="" style={inpStyle('email')} />
              <Err field="email" />
            </div>
            ...
            <div style={{ marginBottom: 14 }}>
              ...
              <div style={{ position: 'relative' }}>
                <input type={showPassword ? 'text' : 'password'} value={form.password} onChange={e => set('password', e.target.value)} onBlur={() => touch('password')} dir="ltr" autoComplete="new-password" placeholder="" style={{ ...inpStyle('password'), paddingInlineEnd: 36 }} />
                <button type="button" onClick={() => setShowPassword(v => !v)} style={{ position: 'absolute', right: 10, top: '50%', transform: 'translateY(-50%)', zIndex: 2, background: 'none', border: 'none', cursor: 'pointer', padding: 0, color: '#d4667a', display: 'flex', alignItems: 'center' }}>
                  {showPassword ? <EyeOff size={16} /> : <Eye size={16} />}
                </button>
              </div>
              <p style={{ fontSize: 11, color: 'var(--txt2)', marginTop: 4 }}>{L.passwordHint}</p>
              <Err field="password" />
            </div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14, marginBottom: 14 }}>
              <div>
                ...
                <input value={form.phone} onChange={e => set('phone', e.target.value.replace(/[^\d]/g, ''))} onBlur={() => touch('phone')} dir="ltr" inputMode="numeric" maxLength={11} style={inpStyle('phone')} />
                <Err field="phone" />
              </div>
              <div>
                <label style={lbl}>{L.rate}{!isSuperAdmin && req}</label>
                <input type="number" min="0" value={form.hourly_rate} onChange={e => set('hourly_rate', e.target.value)} onBlur={() => touch('hourly_rate')} dir="ltr" style={inpStyle('hourly_rate')} />
            ...
            {!isSuperAdmin && (
              <div style={{ marginBottom: 14 }}>
                <label style={lbl}>{L.status}</label>
            ...
            {serverErr && (
              <div style={{ background: '#e0404012', border: '1px solid #e0404028', borderRadius: 10, padding: '10px 14px', color: '#e04040', fontSize: 12, marginTop: 4 }}>
                {serverErr}
              </div>
            )}
          </div>
          ...
          <div style={{ display: 'flex', gap: 10, marginTop: 16 }}>
            <button type="submit" disabled={loading} style={{ background: '#d4667a', border: 'none', borderRadius: 8, padding: '10px 24px', color: '#fff', fontSize: 13, fontWeight: 700, cursor: loading ? 'not-allowed' : 'pointer', opacity: loading ? 0.7 : 1 }}>
              {loading ? L.saving : L.save}
            </button>
            <Link href="/dashboard/staff" style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: 8, padding: '10px 20px', color: 'var(--txt2)', fontSize: 13, fontWeight: 600, textDecoration: 'none', display: 'inline-flex', alignItems: 'center' }}>
              {L.cancel}
            </Link>
          </div>
        </form>
