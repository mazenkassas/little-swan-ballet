...
export default function EventCardActions({ id, name, isRtl }: Props) {
  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', gap: 6 }}>
        <Link href={`/dashboard/events/${id}`} style={{ ...btnBase, color: '#4a90d9', borderColor: '#4a90d928' }}>
          <Eye size={11} />{L.view}
        </Link>
        <Link href={`/dashboard/events/${id}/edit`} style={btnBase}>
          <Pencil size={11} />{L.edit}
        </Link>
        <button onClick={() => setShowConfirm(true)} disabled={pending} style={{ ...btnBase, background: '#e0404012', color: '#e04040', borderColor: '#e0404028' }}>
