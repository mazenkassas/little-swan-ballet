...
const LABELS = {
  en: {
    back: 'Back', title: 'Edit Coach', sub: 'Update coach details',
    ...
    phone: 'Phone Number', rate: 'Hourly Rate (EGP)', status: 'Status',
    ...
    err: {
      ...
      hourly_rate: 'Hourly rate is required', hourly_rate_pos: 'Must be greater than 0',
    },
  },
  ar: {
    back: 'رجوع', title: 'تعديل مدربة', sub: 'تحديث بيانات المدربة',
    ...
    phone: 'رقم الهاتف', rate: 'سعر الساعة (جنيه)', status: 'الحالة',
    ...
    err: {
      ...
      hourly_rate: 'سعر الساعة مطلوب', hourly_rate_pos: 'يجب أن يكون أكبر من صفر',
    },
  },
}
...
export default function EditCoachForm({ coach }: { coach: CoachRecord }) {
  ...
  const [form,         setForm]         = useState({
    ...
    password:    '',
    phone:       coach.phone        || '',
    hourly_rate: coach.hourly_rate != null ? String(coach.hourly_rate) : '',
  })
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const { error: err } = await supabase.from('coaches').update(payload).eq('id', coach.id)
      ...
      <div style={{ padding: '24px 28px' }}>
        <form onSubmit={handleSubmit} noValidate style={{ maxWidth: 640 }}>
          ...
          <div style={{ display: 'flex', gap: 10, marginTop: 16 }}>
            ...
            <Link href="/dashboard/coaches" style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: 8, padding: '10px 20px', color: 'var(--txt2)', fontSize: 13, fontWeight: 600, textDecoration: 'none', display: 'inline-flex', alignItems: 'center' }}>
              {L.cancel}
            </Link>
          </div>
        </form>
